...
# Import state helper from toolset.py (needs careful relative import)
from ..toolset import _log_history_event, _read_toolset_state, _write_toolset_state, _get_history_path # Import specific needed helpers
...
class DeleteFileOrDir(BaseTool):
    ...
    def _run(self, path: str, confirmed_input: Optional[str] = None) -> str:
        ...
        if confirmed_input is None:
            ...
            if str(Path(final_path_str).resolve()) != str(target_path):
...
class OverwriteFile(BaseTool):
    ...
    def _run(self, path: str, new_content: str, confirmed_input: Optional[str] = None) -> str:
        ...
        if confirmed_input is None:
            ...
            except (FileNotFoundError, PermissionError, OSError) as read_e:
                logger.error(f"Error reading original file {target_path} for overwrite proposal: {read_e}", exc_info=True)
                return get_text("tools.overwrite_file.error.read_failed", path=str(target_path), error=read_e)
            except Exception as e:
                logger.error(f"Unexpected error reading file {target_path}: {e}", exc_info=True)
                return get_text("tools.overwrite_file.error.generic", path=str(target_path), error=e)
            max_display_len = 500
            ...
            try:
                ...
                logger.info(f"Successfully overwrote file '{target_path}'")
                 ...
                 if backup_path is None or not backup_path.exists():
                     logger.error(f"Failed to create backup for {target_path}: {op_e}", exc_info=True)
                     return get_text("tools.overwrite_file.error.backup_failed", path=str(target_path), error=op_e)
                 else:
                     logger.error(f"Failed to write new content to {target_path} after backup: {op_e}", exc_info=True)
                     return get_text("tools.overwrite_file.error.write_failed", path=str(target_path), error=op_e)
            except Exception as e:
                 logger.error(f"Unexpected error during overwrite operation for {target_path}: {e}", exc_info=True)
                 return get_text("tools.overwrite_file.error.generic", path=str(target_path), error=e)
            log_data = {
                "operation": "overwrite_confirmed", "path": str(target_path),
        ...
        if confirmed_input is None:
            ...
            try:
                ...
                logger.info(f"Successfully performed find/replace in file '{target_path}'")
                 ...
                 if backup_path is None or not backup_path.exists():
                      logger.error(f"Failed to create backup for {target_path}: {op_e}", exc_info=True)
                      return get_text("tools.find_and_replace_in_file.error.backup_failed", path=str(target_path), error=op_e)
                 elif original_content is None:
                      logger.error(f"Failed to read file {target_path} during find/replace: {op_e}", exc_info=True)
                      return get_text("tools.find_and_replace_in_file.error.write_failed", path=str(target_path), error=op_e)
                 else:
                      logger.error(f"Failed to write file {target_path} during find/replace: {op_e}", exc_info=True)
                      return get_text("tools.find_and_replace_in_file.error.write_failed", path=str(target_path), error=op_e)
            except Exception as e:
                 logger.error(f"Unexpected error during find/replace operation for {target_path}: {e}", exc_info=True)
                 return get_text("tools.find_and_replace_in_file.error.generic", path=str(target_path), error=e)
            log_data = {
                "operation": "find_replace_confirmed", "path": str(target_path),
                "backup_path": str(backup_path) if backup_path else None,
                "summary": final_summary, "find": find_text, "replace": replace_text
            }
            _log_history_event(chat_id, log_data)
...
class Copy(BaseTool):
    ...
    def _run(self, from_path: str, to_path: str, confirmed_input: Optional[str] = None) -> str:
        ...
        if confirmed_input is None:
            ...
            if str(source_path) != str(Path(source_path_str).resolve()):
                 logger.warning(f"Confirmed source path '{source_path}' differs from proposed '{source_path_str}'. Aborting copy.")
...
class Move(BaseTool):
    ...
    def _run(self, from_path: str, to_path: str, confirmed_input: Optional[str] = None) -> str:
        ...
        if confirmed_input is None:
            ...
            dest_path = Path(final_to_str).resolve()
            if str(source_path) != str(Path(source_path_str).resolve()):
                 logger.warning(f"Confirmed source path '{source_path}' differs from proposed '{source_path_str}'. Aborting move.")
...
class RestoreFile(BaseTool):
    ...
    def _run(self, path_or_paths: Union[str, List[str]], backup_id: Optional[str] = None, confirmed_input: Optional[str] = None) -> str:
        ...
        if confirmed_input is None:
            ...
            if not confirmed_input or confirmed_input.lower().strip() in ["no", "cancel", "revert", "n", "abort", "stop"]:
...
class CleanupFileBackups(BaseTool):
    ...
    def _run(self, confirmed_input: Optional[str] = None) -> str:
        ...
        if confirmed_input is None:
            ...
            is_confirmed = (confirmed_input.lower().strip() == 'yes') or \
                           (confirmed_input.strip() == confirm_prompt)
            if not is_confirmed:

# ai_shell_agent/prompts/file_editor_prompt.py
"""
Contains prompt fragments related to using the File Editor toolset.
"""

FILE_EDITOR_TOOLSET_INTRO = """\

You have access to an 'File Editor' toolset for code and text file manipulation.
...
7.  **`submit_editor_input`**: If the editor needs confirmation or clarification during a `request_edit` (indicated by `[FILE_EDITOR_INPUT_NEEDED]`), use this tool to provide the necessary input (e.g., 'yes', 'no', 'all', 'skip', 'don't ask', or specific text).
8.  **`close_file_editor`**: When finished with all editing tasks for the current session, use this to clear the editor's context (included files, history). This makes the other File Editor tools unavailable until you start it again.
